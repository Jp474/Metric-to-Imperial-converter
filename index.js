...
function length() {
    ...
    if (Number(resultInput.value) === 0) {
        lengthPar.textContent = ""
    }  else if (Number(resultInput.value) === 1) {
...
function mass() {
    ...
    if (Number(resultInput.value) === 1) {
        ...
        massPar.textContent = ""
    } else {massPar.textContent = `${resultInput.value} kilograms = ${kilogramVal} pounds | ${resultInput.value} pounds = ${poundVal} kilograms`
}
}